...
describe('HamSwapV2ERC20', () => {
  ...
  let token: Contract
  beforeEach(async () => {
    token = await deployContract(wallet, TestHamSwapV2ERC20, [TOTAL_SUPPLY])
  })

  it('name, symbol, decimals, totalSupply, balanceOf, DOMAIN_SEPARATOR, PERMIT_TYPEHASH', async () => {
    const name = await token.name()
    expect(name).to.eq('HamSwap V2')
    expect(await token.symbol()).to.eq('Ham-V2')
