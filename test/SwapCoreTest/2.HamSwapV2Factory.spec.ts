...
describe('HamSwapV2Factory', () => {
  ...
  async function createPair(tokens: [string, string, string]) {
    const bytecode = `0x${HamSwapV2Pair.evm.bytecode.object}`
    ...
    const pair = new Contract(create2Address, JSON.stringify(HamSwapV2Pair.abi), provider)
  ...
  it('createPair:reverse', async () => {
    await createPair([...TEST_ADDRESSES.slice(0, 2).reverse(), TEST_ADDRESSES[2]] as [string, string, string])
  })
  ...
  it('setFeeTo', async () => {
    await expect(factory.connect(other).setFeeTo(other.address)).to.be.revertedWith('HamSwapV2: FORBIDDEN')
    await factory.setFeeTo(wallet.address)
    expect(await factory.feeTo()).to.eq(wallet.address)
  })

  it('setFeeToSetter', async () => {
    await expect(factory.connect(other).setFeeToSetter(other.address)).to.be.revertedWith('HamSwapV2: FORBIDDEN')
    await factory.setFeeToSetter(other.address)
    expect(await factory.feeToSetter()).to.eq(other.address)
    await expect(factory.setFeeToSetter(wallet.address)).to.be.revertedWith('HamSwapV2: FORBIDDEN')
  })
})
